 ...
 * @param string $custom_type 'taxonomy' or 'post_type'
 *
 * @return array
 */
...
function generate_custom_labels_for_taxonomy( array $config ) {
	return array(
		...
		'choose_from_most_used'      => __( 'Choose from the most used ' . $config['plural_label'], $config['text_domain'] ),
	);
}
...
function generate_custom_labels_for_post_type( array $config ) {
	return array(
		...
		'insert_into_item'      => __( 'Insert into ' . $config['in_sentence_label'], $config['text_domain'] ),
